-- Passkeys / WebAuthn - Tablas y funciones para login por biometría
-- Ejecutar en Supabase: SQL Editor → New query → Pegar y Run
-- Requerido para supakeys (huella, Face ID, Windows Hello, etc.)
...
CREATE INDEX IF NOT EXISTS idx_passkey_credentials_webauthn_user_id ON public.passkey_credentials(webauthn_user_id);
...
CREATE INDEX IF NOT EXISTS idx_rate_limits_lookup ON public.passkey_rate_limits(identifier, identifier_type, endpoint, window_start);
...
CREATE TYPE public.passkey_audit_event AS ENUM (
  ...
  'passkey_removed', 'passkey_updated', 'rate_limit_exceeded', 'challenge_expired', 'counter_mismatch'
);
...
CREATE OR REPLACE FUNCTION public.check_passkey_rate_limit(
  p_identifier TEXT, p_identifier_type VARCHAR(10), p_endpoint VARCHAR(50),
  p_max_attempts INTEGER DEFAULT 10, p_window_minutes INTEGER DEFAULT 1
)
...
CREATE OR REPLACE FUNCTION public.log_passkey_audit_event(
  ...
  p_user_id UUID DEFAULT NULL, p_credential_id TEXT DEFAULT NULL, p_email TEXT DEFAULT NULL,
  p_ip_address INET DEFAULT NULL, p_user_agent TEXT DEFAULT NULL, p_origin TEXT DEFAULT NULL,
  p_metadata JSONB DEFAULT NULL, p_error_code TEXT DEFAULT NULL, p_error_message TEXT DEFAULT NULL
)
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER AS $$
DECLARE v_log_id UUID;
BEGIN
  INSERT INTO public.passkey_audit_log (event_type, user_id, credential_id, email, ip_address, user_agent, origin, metadata, error_code, error_message)
  VALUES (p_event_type, p_user_id, p_credential_id, p_email, p_ip_address, p_user_agent, p_origin, p_metadata, p_error_code, p_error_message)
