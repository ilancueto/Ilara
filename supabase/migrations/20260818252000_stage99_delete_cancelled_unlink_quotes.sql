-- Stage 9.9c — desarmar envío antes de borrar cancelados.
-- No depende de SET CONSTRAINTS (search_path vacío). Forward-only.
...
BEGIN
  ...
  SELECT coalesce(array_agg(o.shipping_quote_id), ARRAY[]::uuid[])
  INTO v_quote_ids
  FROM public.orders o
  WHERE o.id = ANY (v_ids)
    AND o.shipping_quote_id IS NOT NULL;
  ...
  SET
    ...
    total = o.subtotal - o.discount_total
  WHERE o.id = ANY (v_ids);

  UPDATE public.shipping_quotes q
  SET order_id = NULL, consumed_at = NULL
  WHERE q.order_id = ANY (v_ids);

  IF v_quote_ids IS NOT NULL AND array_length(v_quote_ids, 1) IS NOT NULL THEN
    DELETE FROM public.shipping_quotes WHERE id = ANY (v_quote_ids);
  END IF;
