-- Stage 9.9b — borrar cancelados con envío.
-- orders.shipping_quote_id ↔ shipping_quotes.order_id son RESTRICT cruzados.
-- Se diferan en la misma transacción. Forward-only.

ALTER TABLE public.orders
  ALTER CONSTRAINT orders_shipping_quote_fkey DEFERRABLE INITIALLY IMMEDIATE;

ALTER TABLE public.shipping_quotes
  ALTER CONSTRAINT shipping_quotes_order_fkey DEFERRABLE INITIALLY IMMEDIATE;
...
BEGIN
  ...
  SET CONSTRAINTS orders_shipping_quote_fkey, shipping_quotes_order_fkey DEFERRED;
...
COMMENT ON FUNCTION public.delete_cancelled_catalog_orders(uuid) IS
  'Admin: borra pedidos cancelados. Omite cobro aprobado o en revisión. Cierra el ciclo de envío.';
