...
export function useCarrito(showToast: ShowToast) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw && !isExpired) {
        ...
        setCarrito(JSON.parse(raw) as CatalogCartItem[])
  ...
  const agregarAlCarrito = useCallback((producto: PublicCatalogProduct) => {
    setCarrito((prev) => {
  ...
  const agregarComboAlCarrito = useCallback((combo: PublicCatalogCombo) => {
    setCarrito((prev) => {
  ...
  const quitarComboDelCarrito = useCallback(
    (comboId: number) => {
  ...
  const mantenerSoloProductosDisponibles = useCallback(
    (productos: PublicCatalogProduct[], combosIds?: Set<number>) => {
      ...
      setCarrito((prev) => {
        ...
        if (siguiente.length < prev.length) {
          pendingToastRef.current = {
            type: 'info',
            message: 'Se quitaron ítems que ya no están disponibles',
          }
        }
        return siguiente
      })
